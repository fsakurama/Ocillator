% 2 - mass Collision Oscillator with Force
...
%ub_sig = 0.0015*(1-hann(Ns)');
ub_sig = 0.0001*ones(1,Ns);
%ub_sig = 0.005*ones(1,Ns);
%ub_sig = 0.001*sin(2*pi*1*t);
...
for n=1:Ns
    ...
    s0 = M00inv*M1*u + M00inv*M2*um + M00inv*Mf*Fe;
    if u(1)-ub>0 && kc>0 
    ...
    s = M0inv*M1*u + M0inv*M2*um + M0inv*M3*g + M0inv*Mf*Fe;
