% 2 - mass Collision Oscillator with Damping
...
%ub_sig = -0.1*ones(1,Ns);
%ub_sig = -0.001*(1-hann(Ns)');
...
for n=1:Ns
    ...
    M00 = [1+a1+ar1 -a1; -a3 1+a3+a4+ar2];
    M1 = [2*(1-a1) 2*a1; 2*a3 2*(1-a3-a4)];
    M2 = [-2*(1+a1) 2*a1; 2*a3 -2*(1+a3+a4)];
    
    M00inv = (1/((1+a1+ar1)*(1+a3+a4+ar2)-a1*a3)) * [1+a3+a4+ar2 a1; a3 1+a1+ar1];
    ...
    M0 = [1+a1+0.5*(g(1)^2)*a2+ar1 -a1; -a3 1+a3+a4+ar2];
    M3 = [-a2*(2*psim(1)-0.5*g1*(ubp-ubm)) 0; 0 0];
    
    M0inv = (1/((1+a1+0.5*(g(1)^2)*a2+ar1)*(1+a3+a4+ar2)-a1*a3)) * [1+a3+a4+ar2 a1; a3 1+a1+0.5*(g(1)^2)*a2+ar1];
